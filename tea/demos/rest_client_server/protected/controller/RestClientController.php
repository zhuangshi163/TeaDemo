 ...
 * @author leng
 */
class RestClientController extends TeaController{

    public function twitter_daily(){
        //$client = $this->load()->helper('TeaRestClient', true);
        Tea::loadHelper('TeaRestClient');
        $client = new TeaRestClient;
    ...
    public function twitter_follower(){
        ...
        // Check out this Twitter API http://apiwiki.twitter.com/Twitter-Search-API-Method%3A-trends-daily
        $client->connect_to('http://twitter.com/followers/ids/Tea.xml')->get();

        if($client->isSuccess()){
            $data['title'] = 'Twitter Followers\' Id <em>http://twitter.com/followers/ids/Tea.xml</em>';
            $data['content'] = '<br/>HTTP result code: '. $client->resultCode() .'<br/>Received content-type: ' . $client->resultContentType();
    ...
    public function foodById(){
        $server_url = Tea::conf()->APP_URL . 'index.php/api/food/list/'. $this->params['id'].$this->extension;

        $this->load()->helper('TeaRestClient');
        $client = new TeaRestClient;
    ...
    public function foodAllXml(){
        $server_url = Tea::conf()->APP_URL . 'index.php/api/food/list/all.xml';
    ...
    public function foodAllJson(){
        $server_url = Tea::conf()->APP_URL . 'index.php/api/food/list/all.json';
    ...
    public function foodCreateNew(){
        $server_url = Tea::conf()->APP_URL . 'index.php/api/food/create';

        $this->load()->helper('TeaRestClient');
        $client = new TeaRestClient;
        $client->connect_to( $server_url )
               ->accept(TeaRestClient::XML)
               ->data($this->params)
               ->post();
    ...
    public function foodUpdate(){
        $server_url = Tea::conf()->APP_URL . 'index.php/api/food/update';
    ...
    public function foodDelete(){
        $server_url = Tea::conf()->APP_URL . 'index.php/api/food/delete/' . $this->params['id'];

        $this->load()->helper('TeaRestClient');
        $client = new TeaRestClient;
        $client->connect_to( $server_url )->accept( TeaRestClient::XML )->delete();
    ...
    public function foodAdmin(){
        ...
        $server_url = Tea::conf()->APP_URL . 'index.php/api/admin/dostuff';
