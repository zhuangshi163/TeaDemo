...
class BlogController extends TeaController {
	...
	function home() {
        ...
        $pager = new TeaPager(Tea::conf()->APP_URL.'page', $p->count(), 4, 10);
    ...
    function getArchive(){
        ...
        $pager = new TeaPager(Tea::conf()->APP_URL."archive/$year/$month/page", $totalArchive, 4, 10);
        if(isset($this->params['pindex']))
            $pager->paginate(intval($this->params['pindex']));
        else
            $pager->paginate(1);
	...
	function getTag() {
        ...
        $pager = new TeaPager(Tea::conf()->APP_URL."tag/$tag->name/page", $totalPosts, 4, 10);
        if(isset($this->params['pindex']))
            $pager->paginate(intval($this->params['pindex']));
        else
            $pager->paginate(1);
    ...
    function newComment(){
        ...
        // 'skip' is same as TeaValidator::CHECK_SKIP
        if($error = $c->validate( 'skip' ) ){
            ...
            $c->createtime = new TeaDbExpression('NOW()');
            $c->insert();
