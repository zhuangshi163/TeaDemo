...
* Call it from:
* <code>$session = Tea::session("mysite");</code>
...
class TeaSession {
	...
	/**
	* Namespace - Name of Tea session namespace
	*
	* @var string
	*/
	...
	*
	* <code>
	* $session = new TeaSession('mywebsite', $mySessionId)
	* </code>
	*/
	public function __construct($namespace = 'Default', $sessionId = null) {
		...
		// should not be empty
		if ($namespace === '') {
			throw new TeaSessionException('Namespace cant be empty string.');
		}
		// should not start with underscore
		if ($namespace[0] == "_") {
			throw new TeaSessionException('You cannot start session with underscore.');
		}
		// should not start with numbers
		if (preg_match('#(^[0-9])#i', $namespace[0])) {
			throw new TeaSessionException('Session should not start with numbers.');
		}
	...
	*/
	public function __set($name, $value) {
		if ($name === "")
			throw new TeaSessionException("Keyname should not be empty string!");
		if (!$this->_sessionStarted)
			throw new TeaSessionException("Session not started.");
	...
	*/
	public function namespaceUnset($name = null) {
		if (!$this->_sessionStarted) {
			throw new TeaSessionException("Session not started, use TeaSession::start()");
		}
		if (empty($name)) {
	...
	*/
	public function setId($id) {
		if (isset($_SESSION))
			throw new TeaSessionException("Session is already started, id must be set before.");
		if (!is_string($id) || $id === '')
			throw new TeaSessionException("Session id must be string and cannot be empty.");
		if (headers_sent($filename, $linenum))
			throw new TeaSessionException("Headers already sent, output started at " . $filename . " on line " . $linenum);
		session_id($id);
	}
	...
	*/
	public function &get($name) {
		...
		if ($name == '')
			throw new TeaSessionException("Name should not be empty");
	...
	*/
	...
	/**
	* Check if TeaSession variable is stored
	* @return bool
	*/
...
class TeaSessionException extends Exception {
