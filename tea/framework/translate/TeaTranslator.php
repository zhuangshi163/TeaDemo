<?php
/**
 * TeaTranslator class file
 * @package Tea.translate
...
class TeaTranslator {
	...
	* <code>$translator = Tea::translator('Gettext', $this->_basePath . 'languages/en/LC_MESSAGES/en.mo', array());</code>
	*
	* Creating class instance for Csv
	* <code>$translator = Tea::translator('Csv', $this->_basePath . 'languages/en/LC_MESSAGES/main.csv', array('delimiter' => ';',
	*																									   'enclosure' => '"'));</code>
	...
	*/
	...
	public function __construct($adapter, $data, $options=array()) {
		...
		if (!in_array($adapter, $this->_supportedAdapters)) {
			throw new TeaTranslatorException($adapter . " is not supported by TeaTranslator, supported types are: " . implode(', ', $this->_supportedAdapters));
		}
		$this->_setAdapter($adapter);
		if ($data == null) {
			throw new TeaTranslatorException("Data not set!");
		}
		$this->_options = $options;
		// set cache
		if (isset($this->_options['cache'])) {
			if (!in_array($this->_options['cache'], $this->_supportedCacheTypes)) {
				throw new TeaTranslatorException($this->_options['cache'] . " is unsuported cache type, supported cache types are: " . implode(', ', $this->_supportedCacheTypes));
			} else {
				switch ($this->_options['cache']) {
					case "apc":
						$this->_cache = Tea::cache('apc');
						break;
					case "php":
						$this->_cache = Tea::cache('php');
						break;
					case "xcache":
						$this->_cache = Tea::cache('xcache');
						break;
					case "eaccelerator":
						$this->_cache = Tea::cache('eaccelerator');
						break;
					case "file":
						if (isset($this->_options['path'])) {
							$this->_cache = new TeaFileCache($this->_options['path']);
						} else {
							throw new TeaTranslatorException("Cache directory is not set, please add path in options.");
						}
						break;
				}
	...
	*/
	...
	public function translate($messageId, $params=array()) {
		if ($this->_adapter == null) throw new TeaTranslatorException("Adapter not set!");
		$translatedMessage = "";
	...
	private function _loadData($data) {
		...
		switch ($this->_adapter) {
			...
			case 'csv':
				...
				if (!$this->_file) {
					throw new TeaTranslatorException('Error opening translation file "' . $filename . '".');
				}
			...
			case 'gettext':
				...
				if (strtolower(substr(dechex($input[1]), -8)) == "950412de") {
					...
					throw new TeaTranslatorException('"' . $filename . '" is not a valid gettext file!');
				}
...
class TeaTranslatorException extends Exception {
