 ...
 * Use TeaLogWriter to write logs into files. You can output the log message to the debugger tool for development purpose.</p>
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaLog.php 1000 2009-07-7 18:27:22
 * @package Tea.logging
 * @since 1.0
 */
class TeaLog{
    ...
    /**
     * Turn on/off debug mode for TeaLog
     ...
     * @param string $msg
     * @param int $level default TeaLog::TRACE
	...
	public function beginDbProfile($token, $category='database'){
		...
		$this->_logs[] = $log;
		$this->_profiles[$token] = $log;
        $query_index = Tea::db()->getQueryCount();
     ...
     * @param int $level Filter by a log level, eg. TeaLog::INFO
     * @param string $category Filter by a category name
     * @param bool $xml Whether to write as plain text or XML file.
     */
    ...
    protected function writeToFile($data, $filename){
        ...
        if($data!=NULL){
            $mode = 'a+';
            if(isset(Tea::conf()->LOG_PATH))
                $filename = Tea::conf()->LOG_PATH . $filename;
