...
/**
 * TeaCliController is the controller for CLI application should be use with Tea::app('TeaCliApp')->run();
 * @author Richard Myers <richard.myers@hotmail.co.uk>
 * @package Tea.controller
 * @since 1.4 
 */
Tea::loadCore('app/TeaCliApp');
class TeaCliController {
	...
	protected function displayMenu($question, $options, $useNumericalValueSelection = false,
		...
		do {
			...
			$choice = trim(fgets(TeaCliApp::$streamStdIn));
	...
	protected function displayMultiSelectMenu($question, $options,
		...
		do {
			...
			$choices = explode(',', trim(fgets(Tea::app()->streamStdIn)));
	...
	protected function exitApp($code=0, $message=null) {
		TeaCliApp::exitApp($code, $message);
	}
	...
	protected function writeLine($text='', $stream='STD_OUT') {
		TeaCliApp::writeLine($text, $stream);
	}
	...
	public function runCommandLineTask($command, $input='') {
		TeaCliApp::runCommandLineTask($command, $input);
	}
