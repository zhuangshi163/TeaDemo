...
class TeaForm {
    ...
    public function __construct($form) {
        if (is_array($form)) {
            ...
            throw new TeaFormException("Form must be set using array");
        }
    }
    ...
    public function render() {
        ...
        if ($this->_renderFormat == 'array') {
            $formOutput = array(
                'startTeaForm' => $formOpenHtml,
                'endTeaForm' => $formCloseHtml
            );
            ...
            switch ($k[0]) {
                ...
                case 'captcha':
                    ...
                    if (file_exists($k[1]['image'])) {
                        ...
                        $_SESSION['Tea_captcha_' . $element] = $string;
                        if (is_dir($k[1]['directory'])) {
                            imagejpeg($captcha, $k[1]['directory'] . '/' . md5($string) . '.jpg');
                            $elementHtml .= $elementWrappOpen . '<img class="Tea-captcha-image" height="50" width="120" src="' . $k[1]['url'] . md5($string) . '.jpg"/><br/>' .
                                    ...
                                    'class="Tea-captcha-text" />' . $elementWrappClose;
                        } else
    ...
    public function isValid($values) {
        ...
        foreach ($this->_elements as $element => $e) {
            ...
            if (isset($e[0]) && ($e[0] == 'captcha')) {
                $sessionData = (isset($_SESSION['Tea_captcha_' . $element])) ? md5($_SESSION['Tea_captcha_' . $element]) : '';
...
class TeaFormException extends Exception {
