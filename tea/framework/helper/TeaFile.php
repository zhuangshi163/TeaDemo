...
class TeaFile {
     ...
     * @param string $listOnly List only files or folders. Use value TeaFile::LIST_FILE or TeaFile::LIST_FOLDER
	...
	public function getList($path, $listOnly=null, $unit='B', $precision=2){
		...
		foreach($name as $key=>$val){
            if($listOnly==TeaFile::LIST_FILE){
                if(is_dir($val)) continue;
            }
			if ($listOnly==TeaFile::LIST_FOLDER) {
				if(!is_dir($val)) continue;
			}
