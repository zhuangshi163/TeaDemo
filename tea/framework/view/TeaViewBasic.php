...
class TeaViewBasic {
	...
	public function __construct() {
		...
		$this->fileManager = new TeaFile(0777);
		$this->defaultRootViewPath = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . 'view/';
		$this->rootViewPath = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . 'view/';
		$this->rootCompiledPath = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_FOLDER . 'viewc/';
		if (self::$uniqueId == 0) {
			self::$uniqueId = time();
		}
	}
	...
	public function enableSafeVariableAccess($enable = true, $defaultValue = null) {
		...
		TeaViewBasic::$safeVariableResult = $defaultValue;
	}
	...
	protected function block_for($params) {
		...
		$forStmt = '';
		$postForStatement = '';
		$namespace = isset(Tea::conf()->APP_NAMESPACE) ? Tea::conf()->APP_NAMESPACE : 'Tea';
	...
	protected function block_cache($params) {
		if ( preg_match('/\(([^\t\r\n]+?)(([,][ ]*)([0-9]+[ ]*))?[ ]*\)[ ]*$/', $params, $matches)) {
			...
			if (count($matches) == 5) {
				return "<?php if (!Tea::cache('front')->getPart({$matches[1]}, {$matches[4]})): ?>\n<?php Tea::cache('front')->start({$matches[1]}); ?>";
			} else {
				return "<?php if (!Tea::cache('front')->getPart({$matches[1]})): ?>\n<?php Tea::cache('front')->start({$matches[1]}); ?>";
			}
		}
	...
	protected function block_include($params) {
		...
		// Using dynamic variable for the include
			...
			$content  = "<?php\n";
			$content .= "\tif({$filePath} != TeaViewBasic::\$safeVariableResult){\n";
	...
	protected function extractDataPath($str, $ignoreSafe = false) {
		...
		if (!$ignoreSafe && $this->useSafeVariableAccess) {
			return 'TeaViewBasic::is_set_or(' . $result . ')';
		} else {
			return $result;
		}
	}
	...
	public function getUniqueVarId() {
		return '$Tea_view_basic_' . self::$uniqueId++;
	}

	public static function is_set_or(&$var) {
		return (isset($var)) ? $var : TeaViewBasic::$safeVariableResult;
	}
