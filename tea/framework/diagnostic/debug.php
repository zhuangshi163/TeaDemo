...
function getImg(){
    if(isset($_GET['Teadiagnostic_pic']) && file_exists(Tea::conf()->BASE_PATH . 'diagnostic/assets/'.$_GET['Teadiagnostic_pic'])){
        header('Content-Type: image/png');
        header('Content-Length: '.filesize(Tea::conf()->BASE_PATH . 'diagnostic/assets/'.$_GET['Teadiagnostic_pic']));
        ob_clean();
        readfile(Tea::conf()->BASE_PATH . 'diagnostic/assets/'.$_GET['Teadiagnostic_pic']);
        exit;
    }
}
...
function setExceptionHandler($e){
	...
	$err = printVar($e);
	global $errTrace;
	$errTrace = TeaTextHelper::highlightPHP($err);
	if(preg_match_all('/\[file\] \=\> (.+)\n/', $err, $matches)){
		...
		if($e instanceof PDOException){
			foreach($last as $k=>$l){
				if(strpos(str_replace('\\','/',$l), str_replace('\\','/',Tea::conf()->SITE_PATH))===0){
					$lastfile = $l;
					break;
				}
			}
...
function setErrorHandler($errno, $errstr, $errfile, $errline, $errcontext=null){
	...
	//require
	if($errno==2 && strpos($errstr, 'require_once(')===0 && strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Tea::conf()->BASE_PATH).'Tea.php')===0){
		$dmsg = debug_backtrace();
		foreach($dmsg as $k=>$a){
			//if is user file, and must be a load method
			if(strpos(str_replace('\\','/',$a['file']), str_replace('\\','/',Tea::conf()->SITE_PATH))===0
    ...
    else if($errno==2 && strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Tea::conf()->BASE_PATH).'view/TeaView.php')===0){
		$dmsg = debug_backtrace();
		foreach($dmsg as $k=>$a){
            if(strpos(str_replace('\\','/',$a['file']), str_replace('\\','/',Tea::conf()->SITE_PATH))===0
	...
	if(strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Tea::conf()->BASE_PATH))===0){
		$dmsg = debug_backtrace();
		$last = array_pop($dmsg);
		if(isset($last['file'])){
//			$errfile = $last['file'];
//			$errline = $last['line'];
    ...
    $script = TeaTextHelper::highlightPHP($script);
	...
	if(!empty($script)){
        ...
        if(strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Tea::conf()->SITE_PATH.Tea::conf()->PROTECTED_FOLDER).'view')===0){
            $pre = '<pre>';
            $Xpre = '</pre>';
        }
    ...
    $imgloader = Tea::conf()->SUBFOLDER . 'index.php?Teadiagnostic_pic=';

	if (ob_get_level() !== 0) {
		ob_clean();
	}
	$confData = traceVar(Tea::conf());
    ...
    echo "<html><head><title>Tea Diagnostics - $errstr</title>";
    ...
    echo '<script>
		...
		function goVisit(){
			document.location.href="http://Tea.com/";
		}
    </script>';
    ...
    echo "<span id=\"logo\" onclick=\"javascript:goVisit();\">Tea</span><h1>$errstr</h1>";
...
class TeaDebugException extends Exception{
    public $var;
