...
/**
 * TeaSmartModel is a smarter version of TeaModel that provides smart caching of the Model data.
 *
 * <p>The model classes can extend TeaSmartModel for more powerful ORM features which enable you to write shorter codes.
 ...
 * <code>
 * Tea::loadCore('db/TeaSmartModel');
 *
 * class Model extends TeaSmartModel{
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaSmartModel.php 1000 2009-08-28 11:43:26
 * @package Tea.db
 * @since 1.2
 */
class TeaSmartModel{
    ...
    protected static function getCache($id){
        if(self::$cacheMode===null || self::$cacheMode=='file'){
            if($rs = Tea::cache()->getIn('mdl_'.self::$className, $id)){
                //echo '<br>File cached version<br>';
                return $rs;
            }
        }
        else if(self::$cacheMode=='php'){
            if($rs = Tea::cache('php')->getIn('mdl_'.self::$className, $id)){
                //echo '<br>PHP cached version<br>';
                return $rs;
            }
        }
        else{
            if($rs = Tea::cache(self::$cacheMode)->get(Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . $id)){
    ...
    protected static function setCache($id, $value){
        ...
        if(self::$cacheMode===null || self::$cacheMode=='file'){
            Tea::cache()->setIn('mdl_'.self::$className, $id, $value);
        }
        else if(self::$cacheMode=='php'){
            Tea::cache('php')->setIn('mdl_'.self::$className, $id, $value);
        }
        else{
            //need to store the list of Model cache to be purged later on for Memory based cache.
            $keysId = Tea::conf(self::$cacheMode)->SITE_PATH . 'mdl_'.self::$className;
            if($keys = Tea::cache(self::$cacheMode)->get($keysId)){
                $listOfModelCache = $keys->getArrayCopy();
                $listOfModelCache[] = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . $id;
            }else{
                $listOfModelCache = array();
                $listOfModelCache[] = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . $id;
            }
            if(is_array($value))
                Tea::cache(self::$cacheMode)->set(Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . $id, new ArrayObject($value));
            else
                Tea::cache(self::$cacheMode)->set(Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . $id, $value);
            Tea::cache(self::$cacheMode)->set($keysId, new ArrayObject($listOfModelCache));
        }
    }
    ...
    public static function _purgeCache($rmodels=null){
        if(self::$cacheMode===null || self::$cacheMode=='file'){
            Tea::cache()->flushAllIn('mdl_'.self::$className);
        }
        else if(self::$cacheMode=='php'){
            Tea::cache('php')->flushAllIn('mdl_'.self::$className);
        }
        else{
            //loop and get the list and delete those start with the Model name, then delete them
            $keysId = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . 'mdl_'.self::$className;
            if($keys = Tea::cache(self::$cacheMode)->get($keysId)){
                $listOfModelCache = $keys->getArrayCopy();
                foreach($listOfModelCache as $k){
                    //echo '<br>Deleting '. $k .' from memory';
                    Tea::cache(self::$cacheMode)->flush($k);
                }
            }
            Tea::cache(self::$cacheMode)->flush($keysId);
        }
        if($rmodels!==null){
            if(self::$cacheMode===null || self::$cacheMode=='file'){
                foreach($rmodels as $r)
                    Tea::cache()->flushAllIn('mdl_'.get_class($r));
            }else{
                foreach($rmodels as $r){
                    //loop and get the list and delete those start with the Model name, then delete them
                    $keysId = Tea::conf()->SITE_PATH . Tea::conf()->PROTECTED_PATH . 'mdl_'.get_class($r);
                    if($keys = Tea::cache(self::$cacheMode)->get($keysId)){
                        $listOfModelCache = $keys->getArrayCopy();
                        foreach($listOfModelCache as $k){
                            //echo '<br>Deleting '. $k .' from memory';
    ...
    public function count($options=null){
		...
		if (isset($options['distinct']) && $options['distinct'] == true) {
			$options['select'] = 'COUNT(DISTINCT '. $this->_table . '.' . $this->_fields[0] .') as _Teatotal';
		} else {
			$options['select'] = 'COUNT('. $this->_table . '.' . $this->_fields[0] .') as _Teatotal';
		}
        ...
        $value = Tea::db()->find($this, $options);
        $value =  $value['_Teatotal'];
    ...
    public static function _count($model=null, $options=null){
        ...
        $value = Tea::db()->find($model, $options);
        $value = $value['_Teatotal'];
    ...
    public function getOne($opt=null){
        if($opt!==null){
            ...
            if($rs = self::getCache($id)) return $rs;
            $value = Tea::db()->find($this, array('limit'=>1));
        }
    ...
    public static function _getOne($model=null, $opt=null){
        ...
        if($opt!==null){
            ...
            if($rs = self::getCache($id)) return $rs;
            $value = Tea::db()->find($model, array('limit'=>1));
        }
    ...
    /**
     * Retrieve a list of paginated data. To be used with TeaPager
     *
    ...
    public function limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            if($rs = self::getCache($id)) return $rs;
            $value = Tea::db()->find($this, array('limit'=>$limit));
        }
    ...
    public static function _limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            if($rs = self::getCache($id)) return $rs;
            $value = Tea::db()->find(self::$className, $options);
        }else{
            $id = self::toCacheId(new self::$className, 'find', array('limit'=>$limit));
            if($rs = self::getCache($id)) return $rs;
            $value = Tea::db()->find(self::$className, array('limit'=>$limit));
        }
    ...
    public function relate($rmodel, $options=null){
        ...
        if($rs = self::getCache($id)) return $rs;
        $value = Tea::db()->relate($this, $rmodel, $options);
	...
	public function relateMany($rmodel, $opt=null){
        return Tea::db()->relateMany($this, $rmodel, $opt);
    }
	...
	public function relateExpand($rmodel, $opt=null){
        return Tea::db()->relateExpand($this, $rmodel, $opt);
    }
    ...
    public static function _relate($model, $rmodel, $options=null){
        ...
        if($rs = self::getCache($id)) return $rs;
        $value = Tea::db()->relate($model, $rmodel, $options);
    ...
    public function __call($name, $args){
        ...
        if(strpos($name, 'get')===0){
            ...
            if(is_string($field)){
                ...
                if(sizeof($args)>1){
                    ...
                    if($rs = self::getCache($id)) return $rs;
                    $value = Tea::db()->find($obj, $args[1]);
                }else{
                    if(isset($first)){
                        $id = self::toCacheId($obj, 'find', $first);
                        if($rs = self::getCache($id)) return $rs;
                        $value = Tea::db()->find($obj, $first);
                    }else{
                        $id = self::toCacheId($obj, 'find');
                        if($rs = self::getCache($id)) return $rs;
                        $value = Tea::db()->find($obj);
                    }
                }
            }
            else{
        ...
        //$food->relateFoodType_first();    this adds the 'limit'=>'first' to the Options
            ...
            if(sizeof($args)===0){
                Tea::loadModel($relatedClass);
                $id = self::toCacheId($this, 'relate' . $relatedClass);
                if($rs = self::getCache($id)) return $rs;
                $value = Tea::db()->relate( $this, $relatedClass);
            }
                ...
                if(is_array($args[0])){
                    ...
                    if($rs = self::getCache($id)) return $rs;
                    $value = Tea::db()->relate( $this, $relatedClass, $args[0]);
                }else{
                    if(isset($first)){
                        Tea::loadModel($relatedClass);
                        $id = self::toCacheId($args[0], 'relate' . $relatedClass, $first);
                        if($rs = self::getCache($id)) return $rs;
                        $value = Tea::db()->relate( $args[0], $relatedClass, $first);
                    }else{
                        Tea::loadModel($relatedClass);
                        $id = self::toCacheId($args[0], 'relate' . $relatedClass);
                        if($rs = self::getCache($id)) return $rs;
                        $value = Tea::db()->relate( $args[0], $relatedClass);
                    }
                }
            }else{
                Tea::loadModel($relatedClass);
                ...
                if($rs = self::getCache($id)) return $rs;
                $value = Tea::db()->relate( $args[0], $relatedClass, $args[1]);
            }
    ...
    public static function __callStatic($name, $args){
        ...
        //$food->relateFoodType__first();    this adds the 'limit'=>'first' to the Options
            ...
            if(sizeof($args)===0){
                ...
                if($rs = self::getCache($id)) return $rs;
                $value =  Tea::db()->relate( self::$className, $relatedClass);
            }
            else if(sizeof($args)===1){
                if(is_array($args[0])){
                    Tea::loadModel($relatedClass);
                    ...
                    if($rs = self::getCache($id)) return $rs;
                    $value = Tea::db()->relate( self::$className, $relatedClass, $args[0]);
                }else{
                    if(isset($first)){
                        Tea::loadModel($relatedClass);
    ...
    public function insertAttributes($data){
        $this->purgeCache();
        return Tea::db()->insertAttributes($this, $data);
    }
    ...
    public function relatedInsert($rmodels){
        ...
        return Tea::db()->relatedInsert($this, $rmodels);
    }
    ...
    public function update_attributes($data, $opt=NULL){
        $this->purgeCache();
        return Tea::db()->update_attributes($this, $data, $opt);
    }
    ...
    public function relatedUpdate($rmodels, $opt=NULL){
        $this->purgeCache($rmodels);
        return Tea::db()->relatedUpdate($this, $rmodels, $opt);
    }
    ...
    public static function _insertAttributes($model, $data){
        $model->purgeCache();
        return Tea::db()->insertAttributes($model, $data);
    }

    /**
     * Use TeaSmartModel::_insertAttributes() instead.
     * @deprecated deprecated since version 1.3
     */
    ...
    public static function _relatedUpdate($model, $rmodels, $opt=NULL){
        $model->purgeCache($rmodels);
        return Tea::db()->relatedUpdate($model, $rmodels, $opt);
    }
    ...
    public static function _update($model, $opt=NULL){
        $model->purgeCache();
        return Tea::db()->update($model, $opt);
    }
    ...
    public static function _update_attributes($model, $data, $opt=NULL){
        $model->purgeCache();
        return Tea::db()->update($model, $data, $opt);
    }
    ...
    public static function _delete($model, $opt=NULL){
        $model->purgeCache();
        return Tea::db()->delete($model, $opt);
    }
