...
/**
 * TeaSqlMagic is a fast ORM tool which handles relational data.
 *
 * <p>It generates SQL query on the fly and escaping/quoting record values to prevent sql injection.
 * You can do CRUD operations in an easier way with TeaSqlMagic instead of writing queries manually.</p>
 *
 * <p>Database models does not need to extend any classes with TeaSqlMagic. Database table relationships are defined in a
 * centralized way in <i>SITE_PATH/protected/config/db.conf.php</i> instead of defining them in each Model class.</p>
 *
 * <p>Unlike most active records, TeaSqlMagic class does not use any php Magic methods in order to gain higher performance.</p>
 *
 ...
 *
 * <p>TeaSqlMagic is more like a blend between Active record and DataMapper design pattern.</p>
 *
 * <p>Creating an instance of an TeaSqlMagic class does not immediately connect to the RDBMS server.
 *  Use connect() or reconnect() to make the actual connection on demand.</p>
 *
 * Please check {@link http://www.Tea.com/doc/guide/database the Guide} for more details
 * about this class.
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaSqlMagic.php 1000 2009-07-7 18:27:22
 * @package Tea.db
 * @since 1.0
 */
class TeaSqlMagic {
    ...
    public function loadModel($className, $createObj=false){
        ...
        if(empty($this->modelPath)===true) {
            return Tea::loadModel($className, $createObj);
        }
        else {
    ...
    */
    ...
    /**
     * Returns the underlying PDO object used in TeaSqlMagic
     * @return PDO
     */
...
//														echo "<h3>$k2 = $v2</h3>";
     ...
     * Example:
     * <code>
     * Tea::db()->relateMany('Food',array('Recipe','Article','FoodType'))
     * </code>
     *
     * @param mixed $model The model class name or object to be select.
     ...
     * Example:
     * <code>
     * Tea::db()->relateExpand('Recipe', array('Food','Article'))
     * </code>
     *
     * @param mixed $model The model class name or object to be select.
     * @param array $rmodel The related models class names.
