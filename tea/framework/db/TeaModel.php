...
/**
 * TeaModel is the base class for Model classes in Tea that provides useful querying methods.
 *
 * <p>The model classes can extend TeaModel for more powerful ORM features which enable you to write shorter codes.
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: TeaModel.php 1000 2009-08-27 10:28:56
 * @package Tea.db
 * @since 1.2
 */
class TeaModel{
    ...
    public function relate($rmodel, $opt=null){
        return Tea::db()->relate($this, $rmodel, $opt);
    }
    ...
    public static function _relate($model, $rmodel, $opt=null){
        ...
        return Tea::db()->relate($model, $rmodel, $opt);
    }
    ...
    public static function _insertAttributes($model, $data){
        return Tea::db()->insert_attributes($model, $data);
    }
    
    /**
     * Use TeaModel::_insertAttributes() instead.
     * @deprecated deprecated since version 1.3
     */
    ...
    public function limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            //print_r($options);
            return Tea::db()->find($this, $options);
        }
        return Tea::db()->find($this, array('limit'=>$limit));
    }
    ...
    public function getOne($options=null){
        ...
        return Tea::db()->find($this, array('limit'=>1));
    }
    ...
    public static function _limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            return Tea::db()->find(self::$className, $options);
        }
        return Tea::db()->find(self::$className, array('limit'=>$limit));
    }
    ...
    public static function _getOne($model=null, $options=null){
        ...
        return Tea::db()->find($model, array('limit'=>1));
    }
    ...
    public function __call($name, $args){
        ...
        if(strpos($name, 'get')===0){
            ...
            if(is_string($field)){
                ...
                if(sizeof($args)>1){
                    if(isset($first))
                        $args[1]['limit'] = 1;
                    return Tea::db()->find($obj, $args[1]);
                }

                if(isset($first)){
                    return Tea::db()->find($obj, $first);
                }
                return Tea::db()->find($obj);
            }
            else{
                $i=0;
                foreach($field as $f){
        ...
        //$food->relateFoodType_first();    this adds the 'limit'=>'first' to the Options
            ...
            if(sizeof($args)===0){
                return Tea::db()->relate( $this, $relatedClass);
            }
            else if(sizeof($args)===1){
                if(is_array($args[0])){
                    return Tea::db()->relate( $this, $relatedClass, $args[0]);
                }else{
                    if(isset($first)){
                        return Tea::db()->relate( $args[0], $relatedClass, $first);
                    }
                    return Tea::db()->relate( $args[0], $relatedClass);
                }
            }else{
                return Tea::db()->relate( $args[0], $relatedClass, $args[1]);
            }
        }
    }
    ...
    public static function __callStatic($name, $args){
        ...
        //$food->relateFoodType_first();    this adds the 'limit'=>'first' to the Options
            ...
            if(sizeof($args)===0){
                return Tea::db()->relate( self::$className, $relatedClass);
            }
            else if(sizeof($args)===1){
                if(is_array($args[0])){
                    return Tea::db()->relate( self::$className, $relatedClass, $args[0]);
                }else{
                    if(isset($first)){
                        return Tea::db()->relate( $args[0], $relatedClass, $first);
                    }
