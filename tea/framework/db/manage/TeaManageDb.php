...
abstract class TeaManageDb {
	...
	/**
	 * A mapping of Tea generic datatypes to RDBMS native datatypes for columns
	...
	protected $colTypeMapping = array (
		TeaManageDb::COL_TYPE_BOOL		=> null,
    	TeaManageDb::COL_TYPE_SMALLINT	=> null,
    	TeaManageDb::COL_TYPE_INT		=> null,
    	TeaManageDb::COL_TYPE_BIGINT	=> null,
    	TeaManageDb::COL_TYPE_DECIMAL	=> null,
    	TeaManageDb::COL_TYPE_FLOAT		=> null,
    	TeaManageDb::COL_TYPE_CHAR		=> null,
    	TeaManageDb::COL_TYPE_VARCHAR	=> null,
    	TeaManageDb::COL_TYPE_CLOB		=> null,
    	TeaManageDb::COL_TYPE_DATE		=> null,
    	TeaManageDb::COL_TYPE_TIME		=> null,
    	TeaManageDb::COL_TYPE_TIMESTAMP	=> null,
	);
    ...
    public function connect(){
        ...
        try {
            ...
            throw new TeaManageDbException('Failed to open the DB connection', $ex->getMessage());
        }
    }
	 ...
	 * }}
	 *
	 * The field types are defined in the Tea_DB_COL_TYPE_* definitions which are defined in the TeaDbManager.php file
	 *
	...
	public function createTable($table, $cols, $options=null) {
		...
		try {
			...
			throw new TeaManageDbException("Error Creating Table : $table", $ex->getMessage());
		}
	}
	...
	protected function _sqlCreateTable($table, $columnDefinitions, $options=null) {
		...
		foreach($columnDefinitions as $name => $attributes) {
			...
			} catch (TeaDbAdminException $ex) {
				$errors[$name] = $ex->getMessage();
			}
		}
		if ($errors) {
			throw new TeaManageDbException("Could not create table", implode("\n", $errors));
		}
	...
	public function dropTable($table) {
		...
		try {
			...
			throw new TeaManageDbException("Error Dropping Table : $table", $ex->getMessage());
		}
	}
	...
	public function addColumn($table, $name, array $attributes) {
		...
		try {
			...
			throw new TeaManageDbException("Error Adding Column ($name) to Table ($table)", $ex->getMessage());
		}
	}
	...
	public function dropColumn($table, $name) {
		...
		try {
			...
			throw new TeaManageDbException("Error Dropping Column ($name) from Table ($table)", $ex->getMessage());
		}
	}
	...
	protected function buildColumnDefinition($name, $attributes) {
        ...
        if (array_key_exists($type, $this->colTypeMapping) == false) {
            throw new TeaManageDbException('Unrecognised column type specified', "$type is not a recognised database type. Please check TeaDbManager::colTypeMapping for supported column types");
        }

        // get the column type defenition
        switch ($type) {

        	case TeaManageDb::COL_TYPE_CHAR:
        	case TeaManageDb::COL_TYPE_VARCHAR:
	            if ($size < 1 || $size > 255) {
	                throw new TeaManageDbException("Invalid Column Size ($size) for $name", "char and varchar require a column size between 1 and 255");
	            } else {
					$columnDefinition = $this->colTypeMapping[$type] . "($size)";
				}
	            break;

	        case TeaManageDb::COL_TYPE_DECIMAL:
	            if ($size < 1 || $size > 255) {
	                throw new TeaManageDbException("Invalid Column Size ($size) for $name", "decimal requires size to be between 1 and 255");
	            } elseif ($scope < 0 || $scope > $size) {
	                throw new TeaManageDbException("Invalid Column Scope ($scope) for $name", "Scope must be between 0 and the Column Size ($size)");
	            } else {
	...
	protected function checkIdentifier($type, $name) {
        ...
        $len = strlen($name);
        if ($len < 1 || $len > 30) {
            throw new TeaManageDbException("Invalid Identifer for $type : $name", "Identifiers must be between 1 and 30 characters long");
        }
        ...
        if (preg_match($regex, $name) == false) {
            throw new TeaManageDbException("Invalid Identifer for $type : $name", "Identifer must start with a letter a-z and contain only letters, numbers and underscores");
        }

        // must not have two or more underscores in a row
        if (strpos($name, '__') !== false) {
            throw new TeaManageDbException("Invalid Identifer for $type : $name", "Identifier can not contain 2 or more underscores consecutavily");
        }
	}
}


class TeaManageDbException extends Exception {
	/**
	 * An exception thrown by the TeaDbAdmin
